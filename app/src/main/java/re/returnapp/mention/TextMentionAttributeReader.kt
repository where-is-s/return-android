...
/**
 * Reads default attributes that [TextMentionHelper] needs from resources. The attributes read
 * are:
 *
 ...
 */
class TextMentionAttributeReader(context: Context, attrs: AttributeSet?) {
    ...
    init {
        val typedArray = context.obtainStyledAttributes(
            attrs,
            R.styleable.TextMentionHelper,
            0,
            R.style.RoundedBgTextView
        )
        horizontalPadding = typedArray.getDimensionPixelSize(
            R.styleable.TextMentionHelper_roundedTextHorizontalPadding,
            0
        )
        verticalPadding = typedArray.getDimensionPixelSize(
            R.styleable.TextMentionHelper_roundedTextVerticalPadding,
            0
        )
        drawable = typedArray.getDrawableOrThrow(
            R.styleable.TextMentionHelper_roundedTextDrawable
        )
        drawableLeft = typedArray.getDrawableOrThrow(
            R.styleable.TextMentionHelper_roundedTextDrawableLeft
        )
        drawableMid = typedArray.getDrawableOrThrow(
            R.styleable.TextMentionHelper_roundedTextDrawableMid
        )
        drawableRight = typedArray.getDrawableOrThrow(
            R.styleable.TextMentionHelper_roundedTextDrawableRight
        )
        typedArray.recycle()
    }
}
